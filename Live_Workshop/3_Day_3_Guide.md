...
Follow this guide to upgrade your Day 2 project into an AI-powered quiz generator.

---

### Step 1: Install the Google AI SDK
Stop your development server and install the modern Google Gemini SDK:
```bash
npm install @google/genai
```
*(After installing, start your server again with `npm run dev`)*

---

### Step 2: Create the AI Logic (The "Brain")
Create a new folder in `src` called `lib`. Inside it, create `gemini.js`. Copy and paste this code:
...
```
*(Note for instructor: Remember to paste your actual API key where it says `"PASTE_YOUR_API_KEY_HERE"` during your live session before testing!)*

---

### Step 3: Upgrade the Quiz UI (The "Face")
Now, open your existing `src/pages/Quiz.jsx`. We are going to replace the static text area with our dynamic 4-phase game engine (Setup → Loading → Playing → Results).

Replace ALL the code in **`src/pages/Quiz.jsx`** with this:
